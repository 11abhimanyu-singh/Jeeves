...
final class EvidenceRecorder {
    ...
    private func capture(label: String, reachable: Bool, note incoming: String) {
        ...
        if let snap = try? app.snapshot() {
            let screen = CGRect(origin: .zero, size: XCUIScreen.main.screenshot().image.size)
            let tree = A11yDump.node(from: snap, screen: screen)
            ...
            if let data = try? encoder.encode(tree) {
                ...
                fingerprint = labels(of: tree).hashValue
            }
        } else {
        ...
        attach(XCUIScreen.main.screenshot().pngRepresentation,
               name: "\(stem)__screen.png", uti: "public.png")
        ...
        // screen is byte-identical to the last one, say so: an unnoticed stuck
        // sheet turns every later step into a confident photograph of the wrong
