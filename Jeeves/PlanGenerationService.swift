...
enum PlanGenerationService {
    ...
    static func generate(_ req: PlanRequest) async throws -> GeneratedPlan {
        ...
        let body: [String: Any] = [
            "model": model,
            // Fable 5 with adaptive thinking. max_tokens is a generous ceiling,
            ...
            // room for both so the plan never truncates.
            //
            // Fable thinks ALWAYS. "adaptive" is the one explicit setting it
            // accepts — `disabled`, or any budget_tokens, comes back a 400. So
            // this line is not decoration: changing it breaks every plan.
            "max_tokens": 16000,
    ...
    static func planningRules(hasEvents: Bool) -> String {
        ...
        s += "- MINIMUM BLOCK: no activity block is shorter than \(DayPlanner.minActivityMinutes) minutes. This is a hard floor, checked after you answer. If an activity will not fit at \(DayPlanner.minActivityMinutes) min or more, DROP it and leave the time genuinely open — do not schedule the remainder. An activity the routine itself configures shorter than \(DayPlanner.minActivityMinutes) min runs at its own length; the floor bounds shrinking, it does not forbid short activities.\n"
        s += "- IMPORTANT-tier floor: never shrink an Important activity below 50% of its allocated time, and never below the minimum block above, whichever is larger. If fitting the day would force any Important item under that, DROP one Important item entirely (vary which one day to day) so the rest run at full length. One activity done fully beats two done at twenty minutes each — you may even extend a surviving Important item into the freed time rather than leaving it idle.\n"
        ...
        s += "- BREAK: after \(PlanRules.maxContinuousMinutes) minutes of CONTINUOUS work, leave a \(PlanRules.breakMinutes)-minute gap before the next activity. Do not fill it and do not schedule a block in it — it is a gap, not an item. Anything that is not work already counts as the break: lunch, a commute, the gym, an event or free time all reset the count, so never add a gap beside one. This is checked after you answer.\n"
        ...
        s += "DAY WINDOW:\n"
        // The wake hour and the productive start are two different times, and
        // the second one is a setting now. This sentence used to hardcode both
        // as 07:00 and 08:00, so moving the start to 09:00 left an hour nobody
        // owned — and moving it to 06:30 had work beginning before waking.
        let wake = hhmm(DayPreferences.wakeMinute)
        if let window = DayPreferences.morningRoutineWindow {
            s += "- The day STARTS at \(wake) (when sleep ends) — the FIRST block of every plan begins at \(wake). Cover \(hhmm(window.start))–\(hhmm(window.end)) with a morning routine block (kind \"free\", e.g. \"Morning routine — wake, freshen up, breakfast\") unless a real commitment claims that time. Never leave a gap between \(wake) and the first block.\n"
        } else {
            s += "- The day STARTS at \(wake) (when sleep ends) — the FIRST block of every plan begins at \(wake), and it is the first real activity: the productive day starts then too, so there is no morning-routine gap to fill. Never leave a gap between \(wake) and the first block.\n"
        }
